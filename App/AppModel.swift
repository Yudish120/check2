...
final class AppModel: ObservableObject {
    ...
    @Published var copiedDiagnostics = false

    let coreRevision: String = {
        guard let url = Bundle.main.url(
            forResource: "OLCRTC_COMMIT",
            withExtension: "txt"
        ),
        ...
        return String(value.trimmingCharacters(in: .whitespacesAndNewlines).prefix(12))
    }()
    ...
    var connectionStage: String {
        let lower = logs.lowercased()

        if lower.contains("socks5 server listening") {
            return "SOCKS5 готов"
        }

        if lower.contains("session-initiate received") {
            return "Jingle получен"
        }

        if lower.contains("muc joined") {
            return "iPhone вошёл в MUC"
        }

        if lower.contains("joining muc") {
            return "Вход в MUC"
        }
    ...
    func copyDiagnostics() {
        let cleanKey = secret.trimmingCharacters(in: .whitespacesAndNewlines)
        let keyMark: String

        if cleanKey.count >= 12 {
            keyMark = "\(cleanKey.prefix(6))…\(cleanKey.suffix(6))"
        } else {
            keyMark = "invalid"
        }
        ...
        core: \(coreRevision)
        network: \(network.interface), online=\(network.online)
        ...
        setMessage("Диагностика скопирована в буфер.", error: false)
    }
